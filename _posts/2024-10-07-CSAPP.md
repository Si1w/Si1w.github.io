---
title: CSAPP CMU
date: 2024-10-07 14:38:13
tags: C
categories: Computer Science
mathjax: true
---
# Bits, Bytes, and Integers

Everything is bits

- By encoding/interpreting sets of bits in various ways

- Electronic Implementation

Represent all the numbers by bits in computer

## Byte Values

- Byte: 8 bits

Hex | Decimal | Binary
--- | --- | ---
0 | 0 | 0000
1 | 1 | 0001
2 | 2 | 0010
3 | 3 | 0011
4 | 4 | 0100
5 | 5 | 0101
6 | 6 | 0110
7 | 7 | 0111
8 | 8 | 1000
9 | 9 | 1001
A | 10 | 1010
B | 11 | 1011
C | 12 | 1100
D | 13 | 1101
E | 14 | 1110
F | 15 | 1111

## Data Representation

C Data Types \ Bytes | 32-bit | 64-bit | x86-64
--- | --- | --- | ---
char | 1 | 1 | 1
short | 2 | 2 | 2
int | 4 | 4 | 4
long | 4 | 8 | 8
float | 4 | 4 | 4
double | 8 | 8 | 8
long double | - | - | 10/16
pointer | 4 | 8 | 8

Any address is defined by the word size of the machine e.g. 32-bit or 64-bit

## Boolean Algebra

- `&` AND : A & B = 1 when both A and B are 1 (intersection)

- `|` OR : A | B = 1 when either A or B is 1 (union)

- `~` NOT : ~A = 1 when A is 0 (Complement)

- `^` XOR : A ^ B = 1 when A and B are different (Symmetric Difference)

### Trick Operations

`x ^ y` = `~(x & y) & ~(~x & ~y)` = `(~x | ~y) & (x | y)`

## Contrast

`&&`, `||`, `!` (Don't mix up with `&`, `|`, `~`)

- View 0 as "False"

- Any non-zero value is "True"

- Always return 0 or 1 (True or False)

- Early termination

## Shift Operation

- `<<` Left Shift : A << B = A * 2^B

    - Throw away bits on left

    - Fill with 0s on right

- `>>` Right Shift : A >> B = A / 2^B

    - Throw away bits on right

    - Arithmetic Shift : Keep the sign bit the same

    - Logical Shift : Fill with 0s on left

## Numeric Ranges

$w$ is the word size of the machine

- Unsigned Values

    - $0$ to $2^w - 1$

- Signed Values

    - $-2^{w-1}$ to $2^{w-1} - 1$

## Encoding Integers

- Unsigned

    $$B2U_w(x) = \sum_{i=0}^{w-1} x_i \cdot 2^i$$

- Two's Complement

    $$B2T_w(x) = -x_{w-1} \cdot 2^{w-1} + \sum_{i=0}^{w-2} x_i \cdot 2^i$$

    - $x_{w-1}$ is the sign bit

    - $|TMin_w| = |TMax_w| + 1$

## Sign Extension

Make k copies of the sign bit to the left

## Addition & Multiplication

- Operands $w$ bits

- If True Sum: $w+1$ bits Otherwise $w$ bits

- Discard Carry $w$ bits

### Trick operation

`-x = ~x + 1`

`x ^ (x >> 31)` always return non-negative x when x is signed

## Shift Operation

- Multiply

    - Both signed and unsigned $u << k$ gives $u * 2^k$

- Divide

    - Signed: $u >> k$ gives (arithmetic shift) 

        - $\lfloor u / 2^k \rfloor$ if $u \geq 0$

        - $\lceil u / 2^k \rceil$ if $u < 0$


    - Unsigned : $u >> k$ gives $\lfloor u / 2^k \rfloor$ (logical shift)

## Representation in Memory

Memory is a big array of bytes

Addresses Specify the Byte Locations

## Byte Ordering

- Big Endian: Least significant byte has the highest address

- Little Endian: Least significant byte has the lowest address

## Representing Strings

Represent string by a sequence of bytes encoded in ASCII

# Floating Point

## Fractional Binary Numbers

Represents rational numbers: $$\sum_{k=-j}^{i} b_k \times 2^k$$

## IEEE Floating Point

Numerical Form: $$V = (-1)^s * M * 2^E$$

- $s$ is the sign bit (MSB)

- $M$ is the significand in range $[1.0, 2.0)$ (frac field encodes M)

- $E$ is the exponent weights value by power of 2 (exp field encodes E)

|  | 32-bit | 64-bit
--- | --- | ---
s | 1 | 1
E | 8 | 11
M | 23 | 52

## Normalized Values

Exponent coded as a biased value: $$E = Exp - Bias$$

- $Exp$: unsigned value of exp field

- $Bias = 2^{k-1} - 1$, where $k$ is number of exponent bits

## Denormalized Values

- Condition: $Exp = 0$ (exp = 000...0)

- Exponent value: $E = 1 - Bias$

- Significand value: $M = 0.xxx...x$ (xxx...x: bits of frac)

## Special Values

exp | frac | value
--- | --- | ---
111...1 | 000...0 | $\infty$
111...1 | !000...0 | NaN

## Floating Point Operations

Basic idea:

1. compute exact result

2. Make it fit into desired precision

## Rounding

- Towards zero

- Round down ($-\infty$)

- Round up ($+\infty$)

- Nearest even (default)

### Rounding Binary Numbers

- "Even" when least significant bit is 0

- "Half way" when bits to right of rounding position = 100...

## Floating Point Multiplication

$$(-1)^{s1} * M1 * 2^{E1} * (-1)^{s2} * M2 * 2^{E2} = (-1)^{s} * M * 2^E$$

- Sign s: s1 ^ s2

- Significand M: M1 * M2

- Exponent E: E1 + E2

- Fixing

    - If M $\geq$ 2, shift M right and increment E

    - If E out of range, overflow

    - Round M to fit frac precision

## Floating Point Addition

$$(-1)^s1 * M1 * 2^{E1} + (-1)^s2 * M2 * 2^{E2} = (-1)^s * M * 2^E$$

- Sign s: s1 & s2

- Exponent E: max(E1, E2)

- Significand M: M1 + M2

- Fixing

    - If M $\geq$ 2, shift M right and increment E

    - If M < 1, shift M left k positions and decrement E by k

    - Overflow if E out of range

    - Round M to fit frac precision

## Floating Point in C

- `float` : single precision (32-bit)

- `double` : double precision (64-bit)

# Machine-Level Programming

## Definition

- Architecture: The parts of a processor design that one needs to understand or write assembly/machine code

- Microarchitecture: The actual implementation of the architecture

- Code Forms:

    - Machine Code

    - Assembly Code

## View

- PC: Program Counter

- Register file

- Condition codes

- Memory

## Turning C

### Into Object Code

```bash
gcc -Og p1.c p2.c -o p
```

Compiler -> Assembler -> Object Code -> Linker -> Executable

### Into Assembly Code

```bash
gcc -Og -S p1.c
```

## Assembly Data Types

- Integer 1,2,4 or 8 bytes

- Floating Point 4,8 or 10 bytes

- Code: Byte sequences encoding series of instructions

## Assembly Operations

- Perform arithmetic/logic operations

- Move data between registers and memory

- Transfer control between different parts of the program

## Registers

x86-64 Integer Registers

64bit | 32bit | 64 bit | 32bit
--- | --- | --- | ---
%rax | %eax | %r8 | %r8d
%rbx | %ebx | %r9 | %r9d
%rcx | %ecx | %r10 | %r10d
%rdx | %edx | %r11 | %r11d
%rsi | %esi | %r12 | %r12d
%rdi | %edi | %r13 | %r13d
%rsp | %esp | %r14 | %r14d
%rbp | %ebp | %r15 | %r15d

- `%rax` is used to return values from functions

- `%rcx` is used as a counter in loops

- `rdx` is used to save data

- `rbx` is used to hold addresses of data

- `rsi` is used to hold source data

- `rdi` is used to hold destination data

## Moving Data

```assembly
movq Source, Dest
```

Operand Types

- Immediate: Constant integer data

- Register: One of 16 integer registers

- Memory: 8 consecutive bytes of memory at address given by register

## Simple Memory Addressing Modes

Normal (R) Mem[Reg[R]]

- Register R specifies memory address

- Pointer dereferencing in C

Displacement D(R) Mem[Reg[R] + D]

- Register R specifies start of memory region

- Constant displacement D specifies offset

## Complete Memory Addressing Modes

D(Rb, Ri, S) Mem[Reg[Rb] + S * Reg[Ri] + D]

- D: Constant displacement 1, 2, or 4 bytes

- Rb: Base register: Any of 16 integer registers

- Ri: Index register: Any except for %rsp

- S: 1, 2, 4, or 8

## Address Computation Instruction

```assembly
leaq Src, Dest
```

### Some Arithmetic Operations

assembly | Computation
--- | ---
addq Src, Dest | Dest = Dest + Src
subq Src, Dest | Dest = Dest - Src
imulq Src, Dest | Dest = Dest * Src
salq Src, Dest | Dest = Dest << Src
sarq Src, Dest | Dest = Dest >> Src
shrq Src, Dest | Dest = Dest >> Src
xorq Src, Dest | Dest = Dest ^ Src
andq Src, Dest | Dest = Dest & Src
orq Src, Dest | Dest = Dest | Src

## Processor State

- Temporary data (%rax, ...)

- Location of runtime stack (%rsp)

- Location of current code control point (%rip, ...)

- Status of recent tests (CF, ZF, SF, OF)

## Condition Codes

- CF: Carry Flag

- ZF: Zero Flag

- SF: Sign Flag

- OF: Overflow Flag

## Reading Condition Codes

`SetX` instructions

SetX | Condition | Description
--- | --- | ---
sete | ZF | Equal / Zero
setne | ~ZF | Not Equal / Not Zero
sets | SF | Negative
setns | ~SF | Not Negative
setg | ~(SF ^ OF) & ~ZF | Greater (signed)
setge | ~(SF ^ OF) | Greater or Equal (signed)
setl | SF ^ OF | Less (signed)
setle | (SF ^ OF) or ZF | Less or Equal (signed)
seta | ~CF & ~ZF | Above (unsigned)
setb | CF | Below (unsigned)

## Jumping

JX | Condition | Description
--- | --- | ---
jmp | 1 | Unconditional
je | ZF | Equal / Zero
jne | ~ZF | Not Equal / Not Zero
js | SF | Negative
jns | ~SF | Nonnegative
jg | ~(SF ^ OF) & ~ZF | Greater (signed)
jge | ~(SF ^ OF) | Greater or Equal (signed)
jl | SF ^ OF | Less (signed)
jle | (SF ^ OF) or ZF | Less or Equal (signed)
ja | ~CF & ~ZF | Above (unsigned)
jb | CF | Below (unsigned)

## Loop

Do-while

```c
do
    body
while (test);
```
While

```c
while (test) {
    body
}
```

For

```c
for (init; test; update)
    body
```

### Example

```c
long pcount_do(unsigned long x) {
    long result = 0;
    do {
        result += x & 0x1;
        x >>= 1;
    } while (x);
    return result;
}
```

```assembly
pcount_do:
    movl $0, %eax    # result = 0
.L2:                 # do-while loop
    movq %rdi, %rdx  # x -> %rdx
    andl $1, %edx    # t = x & 0x1
    addq %rdx, %rax  # result += t
    shrq %rdi        # x >>= 1
    jne .L2          # if (x) goto .L2
    rep; ret         
```

If we use `-O1` in gcc, then compiler will translate the `While` loop into `do-while` loop, and then transform it into the compatible Goto version. Since `Do-while` loop is more efficient.

## Switch Statement

```c
switch (discriminant) {
    case 1:
        body
    case 2:
        body
    ...
    default:
        body
}
```

### Example

```c
long switch_eg (long x, long y, long z) {
    long w = 1;
    switch (x) {
        case 1:
            w = y * z;
            break;
        case 2:
            w = y / z;
            /* Fall through */
        case 3:
            w += z;
            break;
        case 5:
        case 6:
            w -= z;
            break;
        default:
            w = 2;
    }
    return w;
}
```

```assembly
switch_eg:
    movq %rdx, %rcx
    cmpq $6, %rdi
    ja .L8
    jmp *.L4(, %rdi, 8)
```

Jump table

```assembly
.section .rodata
    .align 8
.L4:
    .quad .L8 # x = 0
    .quad .L3 # x = 1
    .quad .L5 # x = 2
    .quad .L9 # x = 3
    .quad .L8 # x = 4
    .quad .L7 # x = 5
    .quad .L7 # x = 6
```

## Mechanisms in Procedures

- Passing control

- Passing data

- Memory management

- Mechanisms all implemented with machine instructions

## x86-64 Stack

Region of memory managed with stack discipline

Grows toward lower addresses

Register `%rsp` contains lowest stack address

First in Last out

### Push

```assembly
pushq Source
```

Decrement `%rsp` by 8

### Pop

```assembly
popq Dest
```

Increment `%rsp` by 8

## Procedure Control Flow

Use stack to support procedure call and return

Procedure call: `call label`

- Push return address on stack

- Jump to label

Return address

- Address of next instruction after call

Procedure return: `ret`

- Pop address from stack

- Jump to address

### Example

```c
void multstore(long x, long y, long *dest) {
    long t = mult2(x, y);
    *dest = t;
}

long mult2(long a, long b) {
    long s = a * b;
    return s;
}
```

```assembly
multstore:
    # x in %rdi, y in %rsi, dest in %rdx
    pushq %rbx         # Save %rbx
    movq %rdx, %rbx    # Save dest in %rbx
    call mult2         # Call mult2(x, y)
    movq %rax, (%rbx)  # Store return value in dest
    popq %rbx          # Restore %rbx
    ret

mult2:
    # a in %rdi, b in %rsi
    movq %rdi, %rax    # Get value a
    imulq %rsi, %rax   # Compute a * b
    # s in %rax
    ret                # Return
```

## Stack-Based Languages

Languages that support recursion

Stack discipline:

- State for given procedure needed for limited time

Stack allocated in Frames

## Stack Frames

In general, a stack is delimited by two pointers. One is the base pointer (`%rsp`), and the other is the stack pointer (`%rbp`).

Management

- Space allocated when enter procedure

- Deallocated when return

## x86-64/Linux Stack Frame

### Current Stack Frame("Top" to Bottom)

- Arguments (Parameters for function about to call)

- Local variables if can't be allocated in registers

- Saved register context

- Old frame pointer (optional)

### Caller Stack Frame

For all Arguments and return address in stack are Caller Frame

- Return address

    - Pushed by `call` instruction

- Arguments for this call

### Example

```c
long pcount_r(unsigned long x) {
    if (x == 0)
        return 0;
    else
        return (x & 1) + pcount_r(x >> 1);
}
```

```assembly
pcount_r:
    # x in %rdi
    movq $0, %eax      # result = 0
    testq %rdi, %rdi   # Compare x with 0
    je .L6             # If x == 0, return (ZF)
    pushq %rbx         # load %rbx into stack
    movq %rdi, %rbx    # x -> %rbx
    andl $1, %ebx      # t = x & 1
    shrq %rdi          # x >> 1
    call pcount_r      # Recursive call
    addq %rbx, %rax    # result += t
    popq %rbx          # Restore %rbx
.L6:
    rep; ret           # Return
```

## Register Saving Conventions

Caller: calling function

- "Caller Saved": Caller saves temporary values in its frame before the call

Callee: the function being called

- "Callee Saved": Callee saves temporary values in its frame before using. Callee restores them before returning

## x86-64 Linux Register Usage

`%rax` : return value

`%rdi`, ..., `%r9` : arguments

`%r10`, `r11` : Caller-saved

`%rbx`, `%rbp`, `%r12`, `%r13`, `%r14` : Callee-saved

`%rbp` : Callee-saved / May be used as frame pointer (Can be mixed)

`%rsp` : Special form of callee-saved

## Arrays

Basic Principle: `T A[L]`

- Array of data type `T`, length `L`

- Contiguously allocated region of `L * sizeof(T)` bytes in memory

- Identifier `A` can be used as a pointer to arrray element 0: Type `T *`

### Multidimensional Arrays

Declaration: `T A[R][C]`

- `R` rows, `C` columns

- Type `T` requires `K` bytes, Array size is `R * C * K` bytes

### Nested Array Access

Row Vectors: `A[i]`, Starting address `A + i * (C * K)`

Element `A[i][j]`, Address `A + (i * C + j) * K`

```c
int get_a_digit(int index, int dig) {
    return A[index][dig];
}
```

```assembly
get_a_digit:
    # index in %rdi, dig in %rsi
    leaq (%rdi, %rdi, 4), %rax  # %rax = 5 * index
    addl %rax, %rsi             # %rsi = 5 * index + dig
    movl A(, %rsi, 4), %eax     # M[A + 4 * [5 * index + dig]]
    ret
```

`A(, %rsi, 4)` Here we use the complicated addressing mode. The general form is `base(offset, index, scale)`. So `A` is the base, `offset` is 0, `index` is `%rsi`, and `scale` is 4.

## Structures

Structure represented as block of memory.

Fields ordered according to declaration

Compiler determines over size and positions of fields

### Alignment

Primitive data type requires K bytes

Address must be multiple of K

## Memory Layout

from low address to high address

- Stack

- Heap

    - Dynamically allocated as needed

- Data

- Text / Shared Libraries

    - Executable machine instructions and read-only

## Buffer Overflow

Buffer overflow: when exceeding the memory size allocated for the array

```c
typedef struct {
    int a[2];
    double d;
} struct_t;

double fun(int i) {
    s.d = 3.14;
    s.a[i] = 1073741824; /* Out of bound */
    return s.d;
}
```

`fun(0)` -> 3.14

`fun(1)` -> 3.14

`fun(2)` -> 3.1399998664856

`fun(3)` -> 2.00000061035156

`fun(4)` -> 3.14

`fun(6)` -> Segmentation fault

When we access the address cross the boundary of the array, it will result in a change in value of `d`. Also the return address will be changed as well.

If we don't check the input, these kind of errors will appear frequently.

### Code Injection Attacks

Input string contains byte representation of executable code

Overwrite return addresss A with addresss of buffer B

When executes `ret`, will jump to exploit code

### Exploits Based on Buffer Overflows

Buffer overflow bugs can allow remote machines to execute arbitrary code on victim machines

#### Asides: Worms and Viruses

- Worm: A program that can run by itself and propagate a fully working version of itself to other computers

- Virus: Code that adds itself to other programs and does not run independently

Both are designed to spread among computers and to wreak havoc

### Avoiding Buffer Overflow Attacks

1. Avoid overflow vulnerabilities in Code (use library routines that limit string lengths)

    - replace `gets` with `fgets`, `strcpy` with `strncpy`

2. System-Level protections (randomized stack offsets)

3. Stack Canaries

    - idea: Place special value ("canary") on stack just beyond buffer and check for corruption before exiting function

### Return-oriented Programming Attacks

Stack randomization makes it hard to predict buffer location. Making stack noneecutable makes it hard to insert binary code.

**Alternative Strategy**: Use existing code. String together fragments to achieve overall desired outcome --- Construct program from gadgets

## Unions Allocation

Allocate according to the largest element, which can only use one field at a time

## Program Optimization

There's more to performance than asymptotic complexity.

### Limitations of Optimizing Compilers

When in doubt, the compiler must be conservative.

### Generally useful Optimizations

Optimizations that you or the compiler should do regardless of processor / Compiler

- Code Motion: Move code outside loops when possible

- Reduction in Strengths: Replace costly operation with simpler one

- Share Common Subexpression: Reuse portions of expressions

#### Code Motion

```c
void set_row(double *a, double *b, long i, long n) {
    long j;
    for (j=0; j<n; j++)
        a[i*n+j] = b[j];
}
```

Here `n*i` is computed repeatedly, then we can move it out of the loop.

```c
long j;
int ni = n * i;
for (j=0; j < n; j++)
    a[ni+j] = b[j];
```

Reduce frequency with which computation performed if it will always produce same result

#### Reduction in Computation Strength

```c
for (i=0; i<n; i++) {
    int ni = n * i;
    for (j=0; j<n; j++)
        a[ni+j] = b[j];
}
```

Here, try to replace multiplication with addition.

```c
int ni = n * i;
for (j=0; j < n; j++)
    a[ni+j] = b[j];
```

#### Share Common Subexpression

```c
/* Sum neighbors of i, j */
up =    val[(i-1)*n + j  ];
down =  val[(i+1)*n + j  ];
left =  val[i*n     + j-1];
right = val[i*n     + j+1];
sum = up + down + left + right;
```

```c
long inj = i*n + j;
up =    val[inj - n];
down =  val[inj + n];
left =  val[inj - 1];
right = val[inj + 1];
sum = up + down + left + right;
```

### Optimization Blocker

#### Procedure Calls

```c
void lower1(char *s){
    size_t i;
    for (i = 0; i < strlen(s); i++)
        if (s[i] >= 'A' && s[i] <= 'Z')
            s[i] -= ('A' - 'a');
}
```

For each loop iteration, `strlen` is called, then for time complexity, it's $O(n^2)$, which will be very slow for large `n`.

To optimize it, we can move the call to `strlen` outside to compute the length once.

```c
void lower2(char *s){
    size_t i;
    size_t len = strlen(s);
    for (i = 0; i < len; i++)
        if (s[i] >= 'A' && s[i] <= 'Z')
            s[i] -= ('A' - 'a');
}
```

As mentioned before, the strategy of compiler is always keeping conservative. Therefore, some of the optimizations cannot be done by compiler. Programmer should do these optimizations manually.

#### Memory Aliasing

```c
// Compute the sum of each row of nxn matrix a, and store the result in vector b
void sum_rows1(double *a, double *b, long n)
{
    long i, j;
    for (i = 0; i < n; i++)
    {
        b[i] = 0;
        for (j = 0; j < n; j++)
            b[i] += a[i*n + j];
    }
}
```

The corresponding assembly code is as follows:

```assembly
# sum_rows1 loop
.L4:
    movsd   (%rsi, %rax, 8), %xmm0  # Load float number
    addsd   (%rdi), %xmm0           # Float number add
    movsd   %xmm0, (%rsi, %rax, 8)  # Store float number
    addq    $8, %rdi
    cmpq    %rcx, %rdi
    jne     .L4
```

As we can see, the compiler will store `b[i]` and read it repeatedly. Since here, `a` and `b` may refer to the same memory address, which results in a change of value when updating. In fact, it is not necessary to store. Thus, we can use a temporary variable to store the value instead.

```c
void sum_rows2(double *a, double *b, long n)
{
    long i, j;
    for (i = 0; i < n; i++)
    {
        double val = 0;
        for (j = 0; j < n; j++)
            val += a[i*n + j];
        b[i] = val;
    }
}
```

```assembly
.L10:
    addsd   (%rdi), %xmm0   # float number load + add
    addq    $9, %rdi
    cmpq    %rax, %rdi
    jne     .L10
```

### Conditional Branch Prediction

#### Superscalar Processors

A superscalar processor can issue and execute multiple instructions per cycle. The instructions are retrieved from a sequential instruction stream and are usually scheduled dynamically.

#### Pipelined Processors

For those superscalar processors, they will divide the computation into several stages. Then it will pass partial computations from stage to stage.

Which means that, when the processor is working on one instruction, it will also fetch the following instructions.

If the processor is working with an ordered instruction stream, then it will be very simple. However, if the instruction stream is not ordered, for instance, there is a conditional branch jump, then the processor will need to clear the rest of pipeline and refill it with the instructions that may be executed.

In order to avoid the loss of performance, the processor will take a strategy called **branch prediction**. For example, in a loop, as long as the prediction is correct most of the time and only fails on the final iteration when the loop exits, this is acceptable. However, if the processor consistently makes incorrect predictions (for instance, due to unusually complex or confusing code logic), performance can be significantly impacted.

Sometimes branch problems will become the significant problems in the performance of a program. But it is hard to avoid them.

# Memory and Cache

## Random Access Memory(RAM)

RAM is traditionally packaged as a chip. Basic storage unit is normally a cell(one bit per cell). Multiple RAM chips form a memory

RAM comes into two variants:

- Static RAM (SRAM)

- Dynamic RAM (DRAM)

| - | Transistor per bit | Access time | Needs refresh? | Needs EDC? | Cost | Application |
| --- | --- | --- | --- | --- | --- | --- |
| SRAM | 4 or 6 | 1x | No | Maybe | 100x | Cache |
| DRAM | 1 | 10x | Yes | Yes | 10x | Main Memory |

DRAM and SRAM are both volatile, lost information when power is off.

For those non-volatile memories, are called **Read-Only Memory (ROM)**.

Nowadays, we mostly use Flash Memory(EEPROM), with partial (block-level) erase capability.

Firmware programs stored in a ROM (BIOS, controllers for disks, network cards, graphics accelerators etc.)

### Bus

A bus is a collection of parallel wires that carry address, data, and control signals connecting Registers with I/O bridges.

For instance, main memory read A, Those bits travel through the I/O bridge, then the bus interface, and finally reach the register.

## Disk Drive

Another kind of storage device is disk drive.

A disk drive includes:

- Actuator

- Arm

- Spindle

- Platters

- Electronics

- SCSI connector

Disks consist of platters, each with two sides

Each surface consists of concentric rings called **tracks**

Each track consists of **sectors** separated by gaps

### Disk Capacity

**Capacity:** maximum number of bits that can be stored ($1$ GB = $10^9$ bits)

Capacity is determined by these technology factors:

- Recording density(bits/in): number of bits that can be squeezed into a 1 inch segment of track

- Track density(tracks/in): number of tracks that can be squeezed into a 1 inch radial segment

- Areal density(bits/$in^2$): product of recording and track density

Modern disks partition tracks into disjoint subsets called recording zones.

We use average number of sectors/track when computing capacity.

$$Capacity = (\# bytes/sector) * (avg. \# sectors/track) * (\# tracks/suface) * (\# surfaces/platter) * (\# platters/disk)$$

### Disk Access

Data transfer -> Seek -> Rotational latency -> Transfer time

$$T_{access} = T_{seek} + T_{latency} + T_{transfer}$$

- Seek time: Time to position heads over cylinder containing target sector

    - Typical seek time = 3-9ms

- Rotational Latency: Time waiting for first bit of target sector to pass under r/w head

    - Tavg rotation = $1/2 \times 1/RPMS \times 60 sec/min$

- Transfer time: Time to read the bits in the target sector

    - Tag transfer = $1/RPM \times 1/(avg \# sectors/track) \times 60 sec/min$


### Solid State Disks(SSDs)

I/O bus -> Flash translation layer -> Flash memory -> Blocks -> Pages

- Pages: 512 KB to 4 KB, Blocks: 32 to 128 pages

- Data read/written in units of pages

- Page can be written only after its block has been erased

- A block wears out after 100,000 repeated writes

Advantages: No moving parts -> faster, less power, more rugged

Disadvantages: Have the potential to wear out

## Locality

**Principle of Locality:** Programs tend to use data and instructions with addresses near or equal to those they have used recently.

- Temporal locality: recently referenced items are likely to be referenced again in the near future

- Spatial locality: items with nearby addresses tend to be referenced close together in time

```c
sum = 0;
for (i = 0; i < n; i++)
    sum += a[i];
```

`a[i]` is likely to be close to `a[i+1]`, which is spatial locality.

`sum` is likely to be used again in the next iteration, which is temporal locality.

**Claim:** Being able to look at code and get a qualitative sense of its locality is a key skill for a professional programmer.

## Memory Hierarchy

An approach for organizing memory and storage systems known as **Memory Hierarchy**.

0. Register

1. L1 Cache (SRAM)

2. L2 Cache (SRAM)  

3. L3 Cache (SRAM)

4. Main Memory (DRAM)

5. Local secondary storage (local disk)

6. Remote secondary storage (web servers)

from smaller, faster, costlier to larger and slower, cheaper

## Caches

**Cashe:** A smaller, faster storage device that acts as a staging area for a subset of the data in a larger, slower device

### Types of Cache misses

Cold miss occurs since the cache is empty

Conflict miss occurs when the level k cache is large enought, but multiple data objecs all map to the same level k block

Capacity miss occurs when the set of active cache blocks is large to the cache

## Cache Memories

Cache memories are small, fast SRAM-based memories managed automatically in hardware

### Cache Organization (S, E, B)

S = $2^s$ sets

E = $2^e$ lines per set

B = $2^b$ bytes per block

Cache size: $C = S \times E \times B$

### Cache Read

- Locate the set

- Check if any line in set has matching tag

- Yes + line valid -> hit

- No -> miss

- Locate data starting at offset

Address of word: 

| t bits | s bits | b bits |
| --- | --- | --- |
| Tag | Set index | Block offset |
| match tag | find set | offset in block |

#### Example

If B=2, S=4, E=1

`0 00 0`, miss

`0 00 1`, hit

`0 11 1`, miss

`1 00 0`, miss

`0 00 0`, miss

```
       v  Tag   Block
Set 0  1   0    M[0-1]
Set 1  x   x      x
Set 2  x   x      x
Set 3  1   0    M[6-7]
```

If B=2, S=2, E=2

`0 00 0`, miss

`0 00 1`, hit

`0 11 1`, miss

`1 00 0`, miss

`0 00 0`, hit

```
       v   Tag   Block
Set 0  1   00    M[0-1]
Set 0  1   10    M[8-9]
Set 1  1   01    M[6-7]
Set 1  0   x     x
```

### Write Cache

In memory hierarchy, different layers may have the different copy of the same data. When writing data hit in cache, there are two strategies:

- Write-through: Write data to cache and main memory at the same time

- Write-back: Write data to cache only, and only when the cache block is being replaced, write data to main memory

Also, if we write data that miss in cache, there are also two strategies:

- Write-allocate: Allocate a cache block, write data to it, and then write it to main memory

- No-write-allocate: Write data directly to main memory, without allocating a cache block

Normally, **write-allocate** and **no-write-through** are used together, and **write-allocate** and **write-back** are used together.