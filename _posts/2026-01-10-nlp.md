...
To calculate the conditional probability $P(w_{t+j} | w_t; \theta)$, we will use two vectors to represent each word in the vocabulary:$v_w$ when $w$ is the centre word, and $u_w$ when $w$ is a context word. The conditional probability is defined using the softmax function:
